...
**2.4 Refresh and revoke Square tokens.** `refreshOAuthToken` / `revokeOAuthToken` (`square/client.ts:107-124`) have zero callers and `expires_at` is read by no application code. Add a refresh pass to the cron tick, a lazy refresh backstop in `apps/hq/lib/square-runtime.ts`, and a console action for revoke. *Refresh done* (`apps/hq/lib/square-renewal.ts` renews active and inactive connections from the five-minute tick, with the order path as a lazy backstop). *Revoke done* (`apps/hq/lib/square-admin.ts` + Locations → Disconnect Square). *Done:* a location connected 30 days ago still takes cards; `docs/RUNBOOK.md` matches the operated lifecycle.
