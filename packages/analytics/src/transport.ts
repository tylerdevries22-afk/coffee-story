...
export function createAnalyticsTransport(options: AnalyticsTransportOptions): AnalyticsTransport {
  ...
  // Writes are serialized against each other and snapshot the queue at call
  // time, so an interleaved enqueue can never make an older snapshot the last
  // one to land. A failed write is swallowed: telemetry may not break the app.
  const persist = () => {
    if (!store || disposed) return;
    const snapshot = queue.slice();
    persistence = persistence.then(() => store.save(snapshot)).catch(() => undefined);
  };
  ...
  return Object.freeze({
    ...
    flush,
    // Synchronous by contract, so it reports what is in memory. Before
    // hydration lands that can read low; `settled()` is the honest wait.
    queuedCount: () => {
      removeExpired();
      return queue.length;
    },
    // Teardown, not erasure: the whole point of the store is that the queue
    // outlives the process, so disposing must leave the file alone. `purge`
    // is the erasure path.
    dispose: () => {
      disposed = true;
