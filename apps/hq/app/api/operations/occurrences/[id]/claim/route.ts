...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  if (!validUuid(id) || !actionId) return jsonError(400, 'invalid_request', 'A valid occurrence and Idempotency-Key are required.');
  const result = await context.db.rpc('claim_operation_occurrence', {
