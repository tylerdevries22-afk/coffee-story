...
const VALID_PACK = {
  ...
  menu: { categories: [{ id: 'c', title: 'C' }], items: [{ id: 'i', name: 'I', category: 'c', sizes: [] }] },
...
describe('demoPackResponse', () => {
  it('serves the exported pack for a ready, unexpired demo', async () => {
    ...
    const outcome = await demoPackResponse(token, { db, builder: BUILDER });
  ...
  it('checks the token shape before the configuration, and the configuration before the database', async () => {
    ...
    assert.equal((await demoPackResponse('not-a-token', { db, builder: BUILDER })).kind, 'not_found');
    assert.equal((await demoPackResponse(newDemoToken(), { db, builder: null })).kind, 'unavailable');
    assert.equal((await demoPackResponse(newDemoToken(), { db: null, builder: BUILDER })).kind, 'unavailable');
  ...
  it('reports not_found for an expired or removed demo, without leaking which', async () => {
    const expired = await demoPackResponse(newDemoToken(), {
      db: fakeDemoDb({ row: readyRow({ state: 'expired' }) }).db, builder: BUILDER,
    });
    const gone = await demoPackResponse(newDemoToken(), { db: fakeDemoDb({ row: null }).db, builder: BUILDER });
  ...
  it('reports not_found when the stored pack fails demoPackExport\'s own validation', async () => {
    const outcome = await demoPackResponse(newDemoToken(), {
  ...
  it('lets a database failure propagate for the route to turn into a 503', async () => {
    ...
    await assert.rejects(() => demoPackResponse(newDemoToken(), { db, builder: BUILDER }));
  });
});
