import { TRAINING_TRACK_ORDER, type TrainingManifest } from './training';

export type CoreTrainingTrackKey = (typeof TRAINING_TRACK_ORDER)[number];
export type TrainingArtworkUrls = Readonly<Record<CoreTrainingTrackKey, string>>;

const TRACK_PATHS: Record<CoreTrainingTrackKey, string> = {
...
/** Generates deterministic, tenant-neutral module art suitable for immutable Storage keys. */
export function trainingTrackArtworkSvg(trackKey: CoreTrainingTrackKey): string {
...
export function withTrainingArtwork(
  ...
  return {
    ...
    modules: manifest.modules.map((module) => {
      const trackKey = module.trackKey ?? module.slug;
      if (!TRAINING_TRACK_ORDER.includes(trackKey as CoreTrainingTrackKey)) return module;
      return {
        ...module,
        icon: { ...module.icon, url: urls[trackKey as CoreTrainingTrackKey] },
      };
    }),
  };
}
