...
- The production gate is green on main as of `43d58c63`. The failures this baseline first recorded — raw HQ colors in the token audit, an HQ lint precision warning, unresolved app-wall imports in HQ typecheck, 2 of 458 HQ tests — are repaired. `hosted-integration` reporting "skipping" is a design decision rather than a gap: `.github/workflows/verify.yml:165` gates the job to non-pull-request events so pull-request code never executes with the project-creation token, and the same commit receives the hosted gate after it merges to main.
