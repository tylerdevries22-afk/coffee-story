/**
 * What live mode needs from the build environment: the Supabase project the
 * app reads under RLS, and the platform API (the HQ deployment) trusted
 * writes go through. The validators live in @platform/data — one definition
 * of "safe to inline into a public bundle" for every app.
 *
 * Stripe is gone from this list: payments are tender-first through the
 * platform API (pay at pickup now, Square when the brand connects it), so no
 * card SDK key gates live mode any more.
 */
 ...
 * uses to decide whether to show `RuntimeConfigError`. Anything that offers
 * live mode — the startup default, the button on More — asks this rather
 * than a weaker question like "is Supabase set".
 */
...
export function missingLiveConfig(config: MobileLiveConfig): string[] {
  ...
  // The API client pins requests to one exact host; only a localhost dev
  // server may run without the pin.
