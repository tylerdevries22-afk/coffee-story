 ...
 * pipeline in components/customer-telemetry.tsx, which needs a real brand, a
 * session and a consent record this runtime has none of: a demo visitor is
 * anonymous by design, and POST /d/events identifies the site server-side,
 * from the same httpOnly cookie /d/pack.json already reads.
 *
...
import { route, SCREENS } from '@/components/customer-telemetry';
...
export function DemoScreenCapture() {
  ...
  useEffect(() => {
    ...
    reporter.report(screenKeyFor(route(pathname), SCREENS));
  }, [pathname, reporter]);
