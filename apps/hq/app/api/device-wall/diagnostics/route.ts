...
import { runSafeDiagnostics } from '@/lib/device-wall-diagnostics';
...
export async function POST(request: Request) {
  ...
  if (rateLimited(clientIdentity(request), 'device-wall/diagnostics', Date.now(), 5)) {
    return jsonError(429, 'rate_limited', 'Too many diagnostic requests. Try again shortly.');
  }
  ...
  try { return jsonWithCors(await runSafeDiagnostics(db, auth, body), 201); }
  catch (error) {
