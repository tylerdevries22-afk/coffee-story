-- Teach the two server-side manifest readers the schema-3 spelling before any
-- writer emits it.
--
...
--
-- Order matters and this migration has to land first. Both functions below
-- currently read `manifest -> 'modules'` and one of them additionally requires
...
--
-- Both readers therefore accept either spelling rather than switching. A
...
--
-- Lock: two CREATE OR REPLACE FUNCTION statements. No table is touched.
...
-- No assertion of its own: both functions are already asserted by the
-- 20260828163000 link, and neither signature changed.
