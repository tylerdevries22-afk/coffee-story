...
const ALLOWED = new Map<string, string>([
  ...
  ['packages/tenant-config/src/pack-from-setup.ts', 'starter palette stamped into a new tenant pack, same role as tenants/_template/brand.json'],
  ['packages/tenant-config/src/pack-defaults.ts', 'where that starter palette is actually written down: pack-from-setup.ts stamps it, this file defines it, so the literals are the definition rather than a consumer of one'],
  ['apps/hq/components/tier-badge-preview.tsx', 'the preview whose subject is colour; the one literal left is the fallback for a tier colour the tenant typed invalidly -- surface and ink now come from the caller'],
  ...
  ['packages/integrations/src/catalog-audience.ts', "audience providers' official mark colours, which must retain their own identities"],
  ['packages/integrations/src/catalog-commerce.ts', "commerce providers' official mark colours, which must retain their own identities"],
  ['packages/integrations/src/catalog-messaging.ts', "messaging providers' official mark colours, which must retain their own identities"],
  ['packages/integrations/src/catalog-planned.ts', "planned providers' official mark colours, which must retain their own identities"],
